...
public:
    ...
    explicit ReadOnlyText(sf::RenderWindow *window_, std::string text_, const int16_t x_, const int16_t y_,
                          ...
                          const sf::Color background_) : text(std::move(text_)), x(x_), y(y_),
    ...
    void changeBackground(const sf::Color newBackgroundColor) {
...
public:
    explicit Greet(const std::string &s, sf::RenderWindow *window_) {
        textSize = s.size();
...
class Menu {
    sf::RenderWindow *window;
    ReadOnlyText t;
    ReadOnlyText *visibleOptions[10]{};
    std::vector<Option> options;
    int8_t selectedOption = 0;
    int8_t selectionCount;

public:
    explicit Menu(sf::RenderWindow *window_, std::vector<Option> options_) : options(std::move(options_)) {
        window = window_;
        selectionCount = static_cast<int8_t>(options.size());
        for (auto &visibleOption: visibleOptions) {
            visibleOption = nullptr;
        }
        for (int8_t i = 0; i < selectionCount; i++) {
            visibleOptions[i] = new ReadOnlyText{
                window, options[i].optionName, 0, static_cast<int16_t>((i + 5) * 20), 2, window->getSize().x, 20,
                sf::Color::Black, sf::Color::White
            };
        }
        visibleOptions[0]->changeBackground(sf::Color::Green);
    }

    ~Menu() {
        for (const auto &visibleOption: visibleOptions) {
            delete visibleOption;
        }
    }
    ...
    void play() {
        t = ReadOnlyText{
            window,
            "Use the arrow keys to navigate. Use enter to proceed.", 0, 0, 2, window->getSize().x,
            window->getSize().y, sf::Color::Black, sf::Color::White
        };
        while (window->isOpen()) {
            while (const std::optional event = window->pollEvent()) {
    ...
    void manageKey(const sf::Keyboard::Scancode k) {
        int8_t oldSelectedOption = selectedOption;
        if (k == sf::Keyboard::Scancode::Right || k == sf::Keyboard::Scancode::Down) {
            selectedOption--;
            if (selectedOption < 0) {
                selectedOption = static_cast<int8_t>(selectionCount - 1);
            }
        } else if (k == sf::Keyboard::Scancode::Left || k == sf::Keyboard::Scancode::Up) {
            selectedOption++;
            if (selectedOption >= selectionCount) {
                selectedOption = 0;
            }
        } else if (k == sf::Keyboard::Scancode::Enter) {
            auto *g = new Greet(options[selectedOption].GreetLike.greetLikeName,
                                options[selectedOption].GreetLike.window);
            g->play();
            delete g;
        }

        if (oldSelectedOption != selectedOption) {
            visibleOptions[oldSelectedOption]->changeBackground(sf::Color::White);
            visibleOptions[selectedOption]->changeBackground(sf::Color::Green);
        }
    }
...
int main() {
    ...
    // auto *g = new Greet("Welcome to car++!", window);
    // g->play();
    // delete g;

    Option openFromDisk = {
        "OPEN FROM DISK",
        {"you just opened from disk", &window}
    };
    Option newFile = {
        "NEW FILE",
        {"you just created a new file", &window}
    };
    auto *m = new Menu(&window, {openFromDisk, newFile});
    m->play();
    delete m;
