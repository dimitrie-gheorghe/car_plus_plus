...
// Platform-specific helper to determine if the word-jumping modifier is active.
// Windows/Linux use Ctrl, macOS uses Option (Alt).
[[nodiscard]] static bool isWordModifierActive([[maybe_unused]] const bool ctrlPressed) {
#ifdef __APPLE__
    return ctrlPressed || sf::Keyboard::isKeyPressed(sf::Keyboard::Scancode::LAlt) ||
                          sf::Keyboard::isKeyPressed(sf::Keyboard::Scancode::RAlt);
#else
    return ctrlPressed;
#endif
}

// Platform-specific helper for copy/paste/cut/select-all shortcuts.
// Windows/Linux use Ctrl, macOS uses Command (System key).
[[nodiscard]] static bool isShortcutModifierActive([[maybe_unused]] const bool ctrlPressed, [[maybe_unused]] const bool cmdPressed) {
#ifdef __APPLE__
    return cmdPressed;
#else
    return ctrlPressed;
...
void EditableText::manageKey(const sf::Keyboard::Scancode k) {
    ...
    const bool wordModifier = isWordModifierActive(ctrl);
    const bool shortcutModifier = isShortcutModifierActive(ctrl, cmd);

    auto isWordChar = [](const char c) {
    ...
    if (k == sf::Keyboard::Scancode::Right) {
        if (wordModifier) {
            if (newCursor < text.size() && text[newCursor] == '\n') {
                newCursor++;
            } else {
                while (newCursor < text.size() && isWordChar(text[newCursor])) {
                    newCursor++;
                }
                while (newCursor < text.size() && !isWordChar(text[newCursor]) && text[newCursor] != '\n') {
                    newCursor++;
                }
            }
        } else if (*selectionHead + 1 <= text.size()) {
            newCursor++;
        }
        ...
        if (wordModifier) {
            if (newCursor > 0) {
                newCursor--;
                while (newCursor > 0 && !isWordChar(text[newCursor]) && text[newCursor] != '\n') {
                    newCursor--;
                }
                while (newCursor > 0 && isWordChar(text[newCursor - 1])) {
                    newCursor--;
                }
            }
        } else if (static_cast<int16_t>(*selectionHead - 1) >= 0) {
            newCursor--;
        }
...
void EditableText::manageEvent() {
    ...
    if (const auto *enteredText = event->getIf<sf::Event::TextEntered>()) {
        // Filter out Windows control codes generated by Ctrl shortcuts (like Ctrl+A=1, Ctrl+C=3)
        // while preserving legitimate layout controls: Backspace (8), Tab (9), LF (10), CR (13)
        if (const auto unicode = enteredText->unicode;
            unicode < 32 && unicode != 8 && unicode != 9 && unicode != 10 && unicode != 13) {
            return;
        }
        ...
        if (key->scancode == sf::Keyboard::Scancode::LSystem || key->scancode == sf::Keyboard::Scancode::RSystem) {
            cmd = true;
        }
        ...
        if (k->scancode == sf::Keyboard::Scancode::LSystem || k->scancode == sf::Keyboard::Scancode::RSystem) {
            cmd = false;
        }
    }
}
...
EditableText::EditableText(const EditableText &other)
    : Scene(other), text(other.text), currentFilePath(other.currentFilePath),
      ...
      step(other.step), ctrl(other.ctrl), shift(other.shift), cmd(other.cmd), infoBanner(other.infoBanner) {
...
EditableText::EditableText(EditableText &&other) noexcept
    : Scene(std::move(other)), text(std::move(other.text)), currentFilePath(std::move(other.currentFilePath)),
      ...
      step(other.step), ctrl(other.ctrl), shift(other.shift), cmd(other.cmd), infoBanner(std::move(other.infoBanner)) {
    selectionHead = (other.selectionHead == &other.cursorL) ? &cursorL : &cursorR;
